...
class Quadtree{
  ...
  int horizontal, vertical, frameCount, moveDelay, subdivideDelay, level;
  float divideSpeed;
  PVector dest;
  ...
  Quadtree southeast;
  Quadtree southwest;
  Quadtree(Rect boundary, State state, int level) {
  ...
  void spawnTrees() {
    northeast = new Quadtree(new Rect(this.boundary.x + this.boundary.w / 4, this.boundary.y - this.boundary.h / 4, this.boundary.w / 2, this.boundary.h / 2), State.PRE_MOVING, this.level + 1);
    northwest = new Quadtree(new Rect(this.boundary.x - this.boundary.w / 4, this.boundary.y - this.boundary.h / 4, this.boundary.w / 2, this.boundary.h / 2), State.PRE_MOVING, this.level + 1);
    southeast = new Quadtree(new Rect(this.boundary.x + this.boundary.w / 4, this.boundary.y + this.boundary.h / 4, this.boundary.w / 2, this.boundary.h / 2), State.PRE_MOVING, this.level + 1);
    southwest = new Quadtree(new Rect(this.boundary.x - this.boundary.w / 4, this.boundary.y + this.boundary.h / 4, this.boundary.w / 2, this.boundary.h / 2), State.PRE_MOVING, this.level + 1);
  }
  ...
  void update() {
    if (this.state == State.PRE_MOVING) {
      ...
      this.boundary.x = lerp(this.boundary.x, this.dest.x, 0.1);
      this.boundary.y = lerp(this.boundary.y, this.dest.y, 0.1);
